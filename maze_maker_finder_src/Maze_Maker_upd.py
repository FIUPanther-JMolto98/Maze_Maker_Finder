...
def erase_grid(grid, start, end):
    ...
    grid = make_grid(grid.rows, grid.width)
...
def main(win, width, height):
    ...
    #for redraw of buttons; color change when mouse hovers
    button_hi = 40
    button_wid = 120
    ...
    def draw_buttons(surface, height):
        ...
        for i in range(button_rows):
            for j in range(button_cols):
                ...
                buttons.append({'label': button_label, 'pos': (x, y), 'action': button_action, 'color': button_color,
                                'hover': False})
        ...
        for button in buttons:
            ...
            text_rect = text.get_rect(
                center=(button['pos'][0] + button_width // 2, button['pos'][1] + button_height // 2))
            surface.blit(text, text_rect)

        button_hi = button_height
        button_wid = button_width

        pygame.display.update()



    ##### for redrawing buttons, based on the original draw_buttons() method #####################
    # changes COLORS OF TEXT

    def redraw_buttons(surface, height, bttn):
        ...
        for button in buttons:
            ...
            if button['label'] == 'SAVE':
                if bttn == 0:
                    ...
                    text1 = font.render(button['label'], True, YELLOW)
                    border_color = (255, 255, 255)
                else:
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (255, 255, 255))
                    border_color = (255, 255, 255)
            ...
            if button['label'] == 'WIPE':
                if bttn == 9:
                    ...
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (255, 0, 0))
                    border_color = (255, 0, 0)

            if button['label'] == 'DFS':
                if bttn == 1:
                    button_color = (230, 115, 115)
                    text1 = font.render(button['label'], True, YELLOW)
                    border_color = (230, 115, 115)
                else:
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (230, 115, 115))
                    border_color = (230, 115, 115)

            if button['label'] == 'BFS':
                if bttn == 2:
                    button_color = (115, 171, 230)
                    text1 = font.render(button['label'], True, YELLOW)
                    border_color = (115, 171, 230)
                else:
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (115, 171, 230))
                    border_color = (115, 171, 230)

            if button['label'] == 'GRD_L1':
                if bttn == 4:
                    button_color = (153, 230, 115)
                    text1 = font.render(button['label'], True, YELLOW)
                    border_color = (153, 230, 115)
                else:
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (153, 230, 115))
                    border_color = (153, 230, 115)
            ...
            if button['label'] == 'AST_L1':
                if bttn == 7:
                    button_color = (230, 201, 115)
                    text1 = font.render(button['label'], True, YELLOW)
                    border_color = (230, 201, 115)
                else:
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (230, 201, 115))
                    border_color = (230, 201, 115)
            ...
            if button['label'] == 'BI_DFS':
                if bttn == 10:
                    button_color = (192, 115, 230)
                    text1 = font.render(button['label'], True, YELLOW)
                    border_color = (192, 115, 230)
                else:
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (192, 115, 230))
                    border_color = (192, 115, 230)

            if button['label'] == 'CLEAR_SOL':
                if bttn == 11:
                    button_color = (255, 255, 0)
                    text1 = font.render(button['label'], True, ORANGE)
                    border_color = (0, 0, 0)
                else:
                    button_color = (0, 0, 0)
                    text1 = font.render(button['label'], True, (255, 255, 0))
                    border_color = (0, 0, 0)

            rectangle = pygame.Surface((button_width, button_height), pygame.SRCALPHA)
            button_rect = pygame.Rect(button['pos'], (button_width, button_height))
            pygame.draw.rect(rectangle, border_color, button_rect, border_width)
    ...
    while run:
        ...
        btn_num = -1  # for re-draw button method

        pygame.event.post(pygame.event.Event(MOUSE_HOVER))
        ...
        ##################### Mouse hover color change ##################################
            ...
            if (event.type == MOUSE_HOVER or event.type == pygame.MOUSEMOTION): # uses custom event
                ms = pygame.mouse.get_pos()
                button_clicked = handle_button_click(ms, width, WIDTH)

                if button_clicked is not None:
                    pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
                    if button_clicked == 'save':
                        btn_num = 0
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width)).inflate(5, 5)
                    ...
                    if button_clicked == 'dfs':
                        btn_num = 1
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))

                    if button_clicked == 'bfs':
                        btn_num = 2
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))

                    if button_clicked == 'grd_l1':
                        btn_num = 4
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))

                    if button_clicked == 'grd_l2':
                        btn_num = 5
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))

                    if button_clicked == 'ast_l1':
                        btn_num = 7
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))

                    if button_clicked == 'ast_l2':
                        btn_num = 8
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))

                    if button_clicked == 'bi_dfs':
                        btn_num = 10
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))

                    if button_clicked == 'clear_sol':
                        btn_num = 11
                        redraw_buttons(buttons_surface, height, btn_num)
                        win.blit(buttons_surface, (0, width))
